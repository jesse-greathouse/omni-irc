...
(* ----- DIAL (what the connector expects) ----- *)
...
module Net_tcp : DIAL = struct
  ...
  module IO = struct
    ...
    let connect ep =
      ...
      in
      Irc_io_tcp.Tcp_io.IO.connect tcp_ep
    let recv  = Irc_io_tcp.Tcp_io.IO.recv
    let send  = Irc_io_tcp.Tcp_io.IO.send
    let close = Irc_io_tcp.Tcp_io.IO.close
  end
end
...
let () =
  ...
  Lwt_main.run begin
    ...
    in
    let module Conn = Irc_conn.Connector.Make (Net) in
    ...
    let net_task =
      ...
      let send_line s =
        Conn.send tconn ~off:0 ~len:(String.length s) (Bytes.of_string s) >>= fun _ ->
        Lwt.return_unit
      in
